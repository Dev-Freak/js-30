...
const filteredIntentors = inventors.filter(
  (inventor) => inventor.year >= 1500 && inventor.year < 1600
);
console.table(filteredIntentors);

// Array.prototype.map()
// 2. Give us an array of the inventors first and last names
console.log("2. Give us an array of the inventors first and last names");
const fullNamesOfInventors = inventors.map(
...
const accumulatedLifeSpan = inventors.reduce(
  (acc, inventor) => acc + (inventor.passed - inventor.year),
  0
);
console.log(accumulatedLifeSpan);
...
const peopleAlphabeticallyByLastName = people.sort((a, b) => {
  const last = a.split(", ")[0];
  const next = b.split(", ")[0];

  return last > next ? 1 : -1;
});
console.log(peopleAlphabeticallyByLastName);
